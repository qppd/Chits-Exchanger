#!/usr/bin/env python3
"""Fix the CONFIRMED state in yolo_detect_threaded.py"""

# Read the file
with open('yolo_detect_threaded.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the start of CONFIRMED state
start_idx = None
for i, line in enumerate(lines):
    if 'elif detection_state == "CONFIRMED":' in line:
        start_idx = i
        break

if start_idx is None:
    print("ERROR: Could not find CONFIRMED state")
    exit(1)

# Find the end (next elif or last_ir_state line)
end_idx = None
for i in range(start_idx + 1, len(lines)):
    if 'last_ir_state = ir_detected' in lines[i]:
        end_idx = i
        break

if end_idx is None:
    print("ERROR: Could not find end of CONFIRMED state")
    exit(1)

print(f"Found CONFIRMED state from line {start_idx+1} to {end_idx}")
print(f"Replacing {end_idx - start_idx} lines...")

# New CONFIRMED, RELEASING, and DISPENSING states
new_code = '''    elif detection_state == "CONFIRMED":
        ...
        if esp32_msg and "DISPENSING_COMPLETE" in esp32_msg:
            print(f"\\n{'='*60}")
            print(f"✅ DISPENSING COMPLETE (Total time: {elapsed_time:.1f}s)")
            print(f"{'='*60}\\n")
...
# Write back
with open('yolo_detect_threaded.py', 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print(f"✅ Successfully replaced CONFIRMED state handler!")
print(f"   Old code: {end_idx - start_idx} lines")
print(f"   New code includes: CONFIRMED, RELEASING, and DISPENSING states")
print(f"   File updated: yolo_detect_threaded.py")
